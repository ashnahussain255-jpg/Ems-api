...
admin.initializeApp({
  credential: admin.credential.cert(serviceAccount),
  databaseURL: "https://smart-energy-meter-46002-default-rtdb.firebaseio.com"
});
...
// ---------------- FIREBASE LISTENER ----------------
const dbRef = admin.database().ref("data/latest");

console.log("📡 Listening for live data updates from Firebase...");
...
dbRef.on("value", async (snapshot) => {
  ...
  try {
    ...
    console.log(`✅ Stored Data: V=${voltage}, I=${current}, P=${power}`);
  } catch (err) {
    console.error("❌ Mongo Insert Error:", err.message);
  }
}); 
